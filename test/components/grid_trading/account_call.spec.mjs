...
describe(`[${process.pid}] grid_trading: account call`, () => {
  ...
  it('successfully set int param on account 2', async () => {
    const targetFunction = 'setIntParam';
    const entryKey = 'test';
    const entryValue = 1;
    const { stateChanges } = await broadcastAndWait(invokeScript({
    ...
    const accountState = await api.addresses.data(accounts.account2.address);
    expect(accountState).to.deep.include.members([
      {
        key: entryKey,
        type: 'integer',
  ...
  it('change service address', async () => {
    ...
    expect(broadcastAndWait(invokeScript({
      ...
      payment: [],
      chainId,
    }, accounts.user2.seed))).to.be.rejectedWith(errorMessage);
  });
});
