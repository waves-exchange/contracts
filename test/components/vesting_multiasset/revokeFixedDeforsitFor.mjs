 ...
 * } MochaSuiteModified
 * */
describe('vesting_multiasset: revokeFixedDepositFor.mjs', /** @this {MochaSuiteModified} */() => {
  it('should correct do last claim after revoke', async function () {
    ...
    const { stateChanges } = await api.transactions.fetchInfo(revokeFixedDepositFor.id);
    expect(stateChanges.data).to.eql([
      { key: `%s%s__revokedTotal__${this.wxAssetId}`, type: 'integer', value: amount },
      { key: `%s%s%s%d__fixedDeposit__${this.wxAssetId}__${user1}__${releaseBlock}`, type: 'integer', value: 0 },
    ]);
  });
});
