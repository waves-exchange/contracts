...
describe('l2mp_leasing: lease', /** @this {MochaSuiteModified} */() => {
  ...
  it(
    'should be able to lease from locked amount with payment',
    async function () {
      const newCancelAmount = cancelAmount / 10;
      const fromLockedAmount = cancelAmount / 2;
      const paymentAmount = 20e5;
      const leaseAmount = fromLockedAmount + paymentAmount;
      ...
      const newCancelLeaseTx = invokeScript({
        dApp: this.accounts.l2mpLeasing.addr,
        call: {
          function: 'cancelLease',
          args: [{
      ...
      await broadcastAndWait(newCancelLeaseTx);
      ...
      expect(stateChanges.data).to.be.deep.equal([
        ...
        },
        {
          ...
          type: 'string',
          value: `%d%d%d%d__${currentPeriod}__${cancelAmount - (fromLockedAmount - newCancelAmount)}__${nextPeriod}__0`,
        },
        {
          key: `%s%s__userTotalLocked__${this.accounts.user1.addr}`,
          type: 'integer',
          value: initLeaseAmount - cancelAmount - newCancelAmount + leaseAmount,
        },
        {
