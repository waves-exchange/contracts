...
describe(`${process.pid}: voting_emission: finalize`, () => {
  ...
  it('successfull finalize', async function () {
    await broadcastAndWait(data({
      data: [
        { key: '%s__force', type: 'boolean', value: true },
      ],
      additionalFee: 4e5,
      chainId,
    ...
    expect(height).to.be.below(endHeight);
    ...
    expect(dAppState).to.include.deep.members([
      ...
      {
        key: `%s%s%s%d__poolShare__${amountAssetId1}__${priceAssetId1}__0`,
        type: 'integer',
        value: pool1Share,
      },
      {
        key: `%s%s%s%d__poolShare__${amountAssetId2}__${priceAssetId2}__0`,
        type: 'integer',
        value: pool2Share,
      },
    ]);

    const dAppStateObject = dAppState.reduce((acc, cur) => ({ ...acc, [cur.key]: cur }));

    expect(dAppStateObject).to.not.have.property('%s__finalizationStage');
    expect(dAppStateObject).to.not.have.property('%s__force');
  });
});
