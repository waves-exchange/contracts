...
describe('l2mp_staking: stakeForSwapHELPER', /** @this {MochaSuiteModified} */() => {
  it(
    'should be able to stake and set staking node',
    async function () {
  ...
  it(
    'should stake and not set staking node if nodeAddress is empty',
    async function () {
      const stakeAmount = 10e8;
      const price = 1;
      const expectedPrice = price * 10e17;
      ...
      expect(stateChanges.data).to.be.deep.equal([
        {
          key: `%s%s%s__stake__${this.accounts.user1.addr}__${id}`,
          type: 'string',
          value: `%d%d%d%d__0__${expectedPrice}__${stakeAmount}__${expectedLpAmount}`,
        },
        {
          key: '%s__totalLpAmount',
          type: 'integer',
          value: expectedLpAmount * 2,
        },
        {
          key: '%s__totalAssetAmount',
          type: 'integer',
  ...
  it(
    'should reject is i.originCaller is not equal to userAddress',
    async function () {
      const stakeAmount = 10e8;
      ...
      return expect(broadcastAndWait(stakeTx)).to.be.rejectedWith('i.originCaller should be equal to userAddress');
    },
  );
});
