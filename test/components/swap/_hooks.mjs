...
const swapPath = format({ dir: ridePath, base: 'swap.ride' });
...
export const mochaHooks = {
  async beforeAll() {
    ...
    const setFactoryContractToLpStableTx = data({
      additionalFee: 4e5,
      data: [
        {
    ...
    await api.transactions.broadcast(setFactoryContractToLpStableTx, {});
    await waitForTx(setFactoryContractToLpStableTx.id, { apiBase });

    const setFactoryContractToLpTx = data({
      additionalFee: 4e5,
      data: [
        {
          key: '%s__factoryContract',
    ...
    await api.transactions.broadcast(setFactoryContractToLpTx, {});
    await waitForTx(setFactoryContractToLpTx.id, { apiBase });

    const setSwapKeysTx = data({
      additionalFee: 4e5,
      data: [
        {
          key: '%s__factoryContract',
    ...
    await api.transactions.broadcast(setSwapKeysTx, {});
    await waitForTx(setSwapKeysTx.id, { apiBase });
    ...
    console.log('lpStableAddress', address(this.accounts.lpStable, chainId));
    console.log('lpAddress', address(this.accounts.lp, chainId));
    console.log('swap', address(this.accounts.swap, chainId));
    console.log('managerAddress', address(this.accounts.manager, chainId));
    console.log('factoryV2Address', address(this.accounts.factoryV2, chainId));
    console.log('assetsStoreAddress', address(this.accounts.store, chainId));
    console.log('user1', address(this.accounts.user1, chainId));
  },
};
