...
describe('otc_multiasset: withdrawAssetRejectIfAssetPairOrHeightIncorrectly.mjs', /** @this {MochaSuiteModified} */() => {
  it('should reject withdrawAsset', async function () {
    ...
    const expectedRejectMessage = 'otc_multiasset.ride: '
      + 'An asset pair or height is specified incorrectly.';
    ...
    const withdrawAssetTx = invokeScript({
      ...
      call: {
        ...
        args: [
          { type: 'string', value: this.assetAId },
          { type: 'string', value: this.assetBId },
          { type: 'integer', value: heightInKey },
        ],
      },
      chainId,
    }, this.accounts.user1);
