...
describe('lp_stable_check_after_update_script: PutAndGetThanSetScriptAndPutAndGet.mjs', /** @this {MochaSuiteModified} */() => {
  it('should successfully change the state in the same way after changing the script from lp_stable.ride to lp_stable_v2.ride when executing the put and get method', async function () {
    ...
    expect(stateChangesGetAfterPutSecond.data).to.eql([{
      ...
      type: 'string',
      value: `%d%d%d%d%d%d__${usdtAmount}__${usdnAmount}__${expectedOutLpAmt}__${expectedPriceLast}__${heightGetAfterPutSecond}__${timestampGetAfterPutSecond}`,
    }, {
      key: '%s%s__price__last',
      type: 'integer',
      value: expectedPriceLast,
    ...
    // --------------------------------------------------------------------------------------------
    const ridePath = 'ride';
    const lpStableV2Path = format({ dir: ridePath, base: 'lp_stable_v2.ride' });
    const lpStableAddonV2Path = format({ dir: ridePath, base: 'lp_stable_v2_addon.ride' });
